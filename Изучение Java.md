...
```
# Парочка забытых вещей
```java
final int x = 10;  
x = 20; // финальную переменную изменить уже нельзя  
  
String y = null; // значением является ничего, просто НИЧЕГО...  
  
var z = 6.2; // может быть исп. для назначения числ. пер.
```

## База получена. Что же дальше?
# Пора. Открывать. МОДДИНГ!
